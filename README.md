...
### 🐔 유건우 (back-end)
```bash
SSAFY에서 많은 기술들을 빠른 시간에 배우면서 제대로 따라가지 못한 부분들이 많았는데,
관통 프로젝트를 진행하면서 부족한 부분을 실습으로 채우는 시간이었다고 생각합니다.
시간은 넉넉치 않고 어려운 부분도 많았지만, 어찌저찌 무탈하게 마무리 할 수 있었습니다!
모든 기능을 제대로, 정석대로 구현하지는 못했지만, 데이터 엔지니어를 꿈꾸고 있는 저에게는
기본적인 데이터 파이프라인을 구성하기 위해 여러 시도들을 할 수 있어 정말 좋은 경험을
했다고 느끼고 있습니다. 방학이 되면 취업에 도전을 해볼 건데, 해당 프로젝트가 큰 도움이
되길 바랍니다!
잘 모르는 부분도 최선을 다해 끝까지 시도해준 지현이! 정말 고생 많았다!
```
...
```bash
여러모로 많은 것들을 배울 수 있었던 시간이었습니다. 
데이터트랙에 들어와서 데이터 엔지니어링과 웹에 대해 배워보고 싶다고 면접 때 이야기 했었는데 그 두개를 다 활용해볼 수 있었던 프로젝트였습니다. 
이번 프로젝트를 통해 뉴스 데이터를 실시간으로 수집하고 분석하는 전 과정을 직접 설계하고 구현해보며 데이터 파이프라인의 흐름을 이해할 수 있었습니다.
여기와서 깃을 처음 써봐서 잘못 건드릴까 걱정이 많았는데 옆에서 팀원이 잘 알려주고 문제가 일어났을 떄 함께 해결해줘서 무사히 마무리할 수 있었습니다. 
개발로 처음 시작해보는 프로젝트였는데 얻어가는게 많았던 프로젝트인 거 같습니다! 
```
